...
public class NestedTrianglesGenerator implements LevelGenerator {
    ...
    private List<Node> createNestedNodes(int width, int height, int layers) {
        ...
        for (int layer = 0; layer < layers; layer++) {
            ...
            nodes.add(new BasicNode(layer * 3, top.x, top.y));
            nodes.add(new BasicNode(layer * 3 + 1, left.x, left.y));
            nodes.add(new BasicNode(layer * 3 + 2, right.x, right.y));
        }
    ...
    private List<Edge> connectTriangles(List<Node> nodes, int layers) {
        ...
        for (int layer = 0; layer < layers; layer++) {
            ...
            // Triangle edges
            edges.add(new BasicEdge(nodes.get(baseIdx), nodes.get(baseIdx + 1))); // Top-left
            edges.add(new BasicEdge(nodes.get(baseIdx + 1), nodes.get(baseIdx + 2))); // Left-right
            edges.add(new BasicEdge(nodes.get(baseIdx + 2), nodes.get(baseIdx))); // Right-top
            ...
            if (layer < layers - 1) {
                ...
                edges.add(new BasicEdge(nodes.get(baseIdx), nodes.get(innerBase))); // Top
                edges.add(new BasicEdge(nodes.get(baseIdx + 1), nodes.get(innerBase + 1))); // Left
                edges.add(new BasicEdge(nodes.get(baseIdx + 2), nodes.get(innerBase + 2))); // Right

                // Add cross connections for visual interest
                edges.add(new BasicEdge(nodes.get(baseIdx), nodes.get(innerBase + 1)));
                edges.add(new BasicEdge(nodes.get(baseIdx + 1), nodes.get(innerBase + 2)));
                edges.add(new BasicEdge(nodes.get(baseIdx + 2), nodes.get(innerBase)));
            }
        }
